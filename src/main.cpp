...
//
//    vector<string> str = expand({"Q_o", "P_o"}, 2);
//
//    cout << str[0] << str[1] << endl;
//    return 0;
    
    // Heart, veins, lungs
//    RCR * PA = new RCR("Pulm. Arteries", {"Q_pv"}, {"P_rv", "P_pa"}, {"Q_pa"}, {"P_pa"}, {0.003, 4.3});
//    PA->setHasValve(true);
//
//    RCR * PV = new RCR("Pulm. Veins", {"Q_li"}, {"P_pa", "P_pv"}, {"Q_pv"}, {"P_pv"}, {0.08, 8.4});
//
//    RCR * veins = new RCR("veins", {"Q_ri"}, {"P_o_5", "P_v"}, {"Q_v"}, {"P_v"}, {1.0, 15.0});
//    veins->setHasValve(true);
//
//    heart * hrt = new heart({"Q_pa", "Q_lo"}, {"C_ra", "C_rv", "C_la", "C_lv", "P_ra", "P_rv", "P_la", "P_lv", "P_v", "P_pv"}, {"C_ra", "C_rv", "C_la", "C_lv", "Q_ri", "Q_rav", "Q_li", "Q_lav"}, {"C_ra", "C_rv", "C_la", "C_lv", "P_ra", "P_rv", "P_la", "P_lv"}, {0.8, 2.5, 0.13, 1.3, 0.07, 0.61, 0.5, 0.74, 0.3, 0.06, 0.015, 0.005, 0.01, 0.01});
//
//    RCR * art = new RCR("Arteries", {"Q_kid_1"}, {"P_lv", "P_a"}, {"Q_lo"}, {"P_a"}, {0.006, 2.0});
//    art->setHasValve(true);
    
    // Myogenic arteries
    
    // C_pass, C_pass_prime, D_0, C_act, C_act_prime, C_act_primeprime, C_rbc_meta, C_ATP_0, C_myo, C_shear, C_symp, C_tone_primeprime, tau_d, tau_a, mu, nParallel
//    myogenic * myorgan = new myogenic("myoOrgan", {"Q_o"}, {"D_o", "A_o", "P_o"}, {"T_total_o", "A_total_o", "T_o"}, {"D_o", "A_o"}, {1154, 7.846, 173.1, 2671, 0.6709, 0.2765, 30, 0.5, 0.0079, 0.0258, 3.0, 0, 1, 20, 0.0227, 6000});
//
//    myoRC * myoflow = new myoRC("organ", {"Q_v"}, {"P_a", "P_o", "D_o"}, {"Q_o"}, {"P_o"}, {1.0, 6000, 2.27, 1500000, 0.005});
...
//    double y0[] = {2.0, 20.0};
    double reltol = 1e-6;
    double abstol[] = {1e-6, 1e-6, 1e-6, 1e-6, 1e-6, 1e-6, 1e-6, 1e-6, 1e-6, 1e-6, 1e-6, 1e-6, 1e-6, 1e-6, 1e-6, 1e-6, 1e-6, 1e-6, 1e-6, 1e-6, 1e-6, 1e-6};
