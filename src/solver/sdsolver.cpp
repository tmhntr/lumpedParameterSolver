...
sdsolver::sdsolver(component* m, double INITIAL_CONDITIONS[], double reltol1, double abstol1[], double* deltat)
    : solver(m)
{
    setDeltaT(deltat);
    ...
    for (int i = 0; i < NEQ; i++)
        Ith(abstol, i + 1) = (realtype)abstol1[i];
